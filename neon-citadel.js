/* Neon Citadel — Flagship Tower Defence
   - Grid/maze builder, bright neon bloom visuals, elemental towers and synergies
   - Large variety of enemies, elite types, and utility towers (boosters, debuffs)
   - Creative mechanics: conduits (connect towers for bonus), elemental interactions (fire/ice/void)
   - API-compatible: init(canvas,difficulty), update(), draw(ctx), click(x,y), startWave(), setBuild(k), upgrade(), sell()
*/

const NeonCitadel = {
  canvas:null, ctx:null,
  gridW:20, gridH:15, cell:40,
  ...
  update: function() {
    ...
    // towers fire with synergy (conduits: adjacent same-type towers get small bonus)
    this.towers.forEach(t => {
      if (t.cd > 0) t.cd--;
      else {
        // compute local conduit bonus (adjacent same-type towers)
        let conduit = 1.0;
        ...
        if (targets.length) {
          ...
          if (t.aoe) {
            // pulse: damage scaled by conduit
            ...
            // single projectile with elemental interactions
  ...
  // handle elemental interactions (fire = damage over time, frost = slow, void = leech)
  applyElementalDamage: function(e, dmg, element) {
    ...
    if (element === 'thermal') {
      ...
      this.money += Math.floor(final * 0.02); // small leech economy
    } else {
      e.hp -= final;
    }
  ...
  draw: function(ctx) {
    ...
    // highlight path according to flow
  ...
  startWave: function() {
    ...
    this.active = true;
    // creative waves: include elites, shielders, fast swarms, leechers
  ...
  click: function(px, py) {
    ...
    if (this.build) {
      ...
      // place and ensure path exists
      this.grid[gx][gy] = 1;
      if (!this.calcFlow()) { this.grid[gx][gy] = 0; return; } // blocked
      this.money -= def.cost;
  ...
  // BFS flow: compute 'next' for each grid cell toward end
  calcFlow: function() {
    ...
    // mark towers as blockers (they occupy cells)
    ...
    // reconstruct for every reachable node a 'next' pointer
    this.flow = {};
    for (let key in came) {
  ...
  applyElementalDamage: function(e, dmg, element) {
    // same mechanic as in applyElementalDamage above — simple fallback if element absent
    e.hp -= dmg;
    if (e.hp <= 0) { e.dead = true; this.money += e.val; }
  },

  spawn: function(d) {
    this.enemies.push({x:this.start.x*this.cell+this.cell/2, y:this.start.y*this.cell+this.cell/2, hp:d.hp, maxHp:d.hp, spd:d.spd, type:d.type, color:d.color, val:d.val, slow:0});
  },
...
// Provide fallback global name to match page expectations
