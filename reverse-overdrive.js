...
(function(global){
  const Reverse = (function(){
    ...
    const UNITS = {
      runner: { name:'RUNNER', cost:30, color:'#6ee7b7', hp:55, spd:3.8, dmg:7, range:26, rate:11, breach:14, role:'runner', desc:'Fast breach unit focused on core damage.' },
      raider: { name:'RAIDER', cost:55, color:'#7dd3fc', hp:80, spd:3.0, dmg:12, range:30, rate:15, breach:12, role:'breaker', desc:'Skirmisher that harasses nearby towers.' },
      brute:  { name:'BRUTE',  cost:90, color:'#fb7185', hp:210, spd:1.8, dmg:26, range:38, rate:24, breach:20, armor:0.3, role:'breaker', desc:'Armored breaker with heavy tower damage.' },
      siege:  { name:'SIEGE',  cost:160, color:'#fbbf24', hp:300, spd:1.3, dmg:36, range:60, rate:30, breach:32, bonus:1.6, role:'breaker', desc:'Siege rig that melts fortified nodes.' },
      hacker: { name:'HACKER', cost:130, color:'#c77dff', hp:115, spd:2.3, dmg:5, range:95, rate:20, breach:12, stun:80, role:'breaker', desc:'Disables towers briefly while attacking.' },
      bomber: { name:'BOMBER', cost:140, color:'#f97316', hp:130, spd:2.5, dmg:0, range:20, rate:1, breach:16, explode:120, role:'breaker', desc:'Explodes on contact, damaging nearby towers.' },
      ghost:  { name:'GHOST',  cost:110, color:'#d1fae5', hp:70, spd:3.4, dmg:9, range:26, rate:12, breach:18, stealth:0.55, role:'runner', desc:'Harder to target; slips past defenses.' },
      medic:  { name:'MEDIC',  cost:100, color:'#a7f3d0', hp:110, spd:2.6, dmg:5, range:80, rate:20, breach:8, heal:10, role:'support', desc:'Repairs allied attackers in a radius.' }
    };
    ...
    const ABILITIES = {
      emp: { name:'EMP', cost:180, cooldown:600, radius:180, stun:140 },
      overclock: { name:'OVERCLOCK', cost:140, cooldown:520, duration:300, dmg:0.55, spd:0.45 },
      decoy: { name:'DECOY SWARM', cost:120, cooldown:450, count:8, type:'runner' },
      strike: { name:'ORBITAL STRIKE', cost:220, cooldown:900, radius:160, dmg:160 }
    };
    ...
    function applyUpgradeToUnit(unit) {
      ...
      unit.maxHp *= 1 + (upgradeState.hp * 0.25);
    ...
    function update() {
      ...
      if (aiCooldown === 0 && aiCredits >= 120) {
        ...
        aiCooldown = built ? Math.max(6, 22 - wave * 1.3) : 10;
      }
      if (aiUpgradeCooldown === 0 && aiCredits >= 90) {
        const upgraded = attemptUpgradeTower();
        aiUpgradeCooldown = upgraded ? Math.max(60, 160 - wave * 5) : 90;
      }
