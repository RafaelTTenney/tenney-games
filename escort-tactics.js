...
/* ASSAULT PROTOCOLS: ESCORT TACTICS */
(function(global){
  const Game = (function(){
    const CELL = 40;
    ...
    const UNITS = {
      scout: { name:'SCOUT', cost:40, color:'#22d3ee', hp:70, spd:2.6, dmg:6, range:70, rate:18, supply:1, tier:1, desc:'Fast escort that scouts and pokes towers.' },
      guard: { name:'GUARD', cost:70, color:'#a7f3d0', hp:140, spd:2.0, dmg:12, range:80, rate:20, supply:2, tier:1, desc:'Balanced convoy guard unit.' },
      medic: { name:'MEDIC', cost:85, color:'#c4f2ff', hp:120, spd:1.9, dmg:4, range:90, rate:24, heal:10, supply:2, tier:1, desc:'Support drone that repairs escorts.' },
      breaker: { name:'BREAKER', cost:130, color:'#fb7185', hp:230, spd:1.6, dmg:28, range:85, rate:26, armor:0.35, supply:3, tier:2, desc:'Armored breaker for heavy defenses.' },
      sniper: { name:'SNIPER', cost:140, color:'#f8fafc', hp:90, spd:1.7, dmg:32, range:170, rate:36, supply:2, tier:2, desc:'Long-range overwatch escort.' },
      ward: { name:'WARD', cost:160, color:'#fbbf24', hp:170, spd:1.4, dmg:8, range:120, rate:28, shield:0.2, supply:3, tier:3, desc:'Shield drone that dampens tower damage.' }
    };

    const TOWER_TYPES = {
      rail: { name:'RAIL', cost:140, color:'#f97316', range:200, dmg:10, rate:10, hp:150, beam:true },
      mortar:{ name:'MORTAR', cost:170, color:'#fbbf24', range:190, dmg:26, rate:46, hp:170, aoe:70 },
    ...
    const ABILITIES = {
      emp: { name:'EMP', cost:200, cooldown:820, radius:180, stun:200 },
      overclock: { name:'OVERCLOCK', cost:180, cooldown:700, duration:320, dmg:0.35, spd:0.3 },
      decoy: { name:'DECOY FLOCK', cost:150, cooldown:620, count:3, type:'scout' },
      strike: { name:'ARTILLERY', cost:260, cooldown:980, radius:150, dmg:150 }
    };

    const GAME_ID = 'assault-escort';
    ...
    let supplyCap = 8;
    let supplyUsed = 0;
    let unlocks = { tier1: true, tier2: false, tier3: false };
    ...
    function reset(){
      ...
      supplyCap = 8;
      supplyUsed = 0;
      unlocks = { tier1: true, tier2: false, tier3: false };
      units = [];
      towers = [];
      projs = [];
      particles = [];
    ...
    function isUnitUnlocked(type){
      ...
      if (def.tier === 2) return unlocks.tier2;
      if (def.tier === 3) return unlocks.tier3;
      return false;
    }
    ...
    function spawnUnit(type){
      ...
      const unit = {
        x: carrier.x + Math.cos(angle) * 30,
        y: carrier.y + Math.sin(angle) * 30,
        angle,
        orbit: 40 + Math.random() * 20,
        hp: def.hp,
        maxHp: def.hp,
        spd: def.spd,
        baseSpd: def.spd,
    ...
    function startWave(){
      inTransit = true;
      if (!waypoints.length) {
        waypoints.push({ x: canvas.width - 120, y: canvas.height / 2 });
      }
      money += 160 + wave * 60;
    ...
    function updateWaypoints(){
      if (!inTransit || !waypoints.length) return;
    ...
    function updateGates(){
      gates.forEach(g => {
        ...
        if (Math.hypot(carrier.x - g.x, carrier.y - g.y) < g.radius) {
          g.active = false;
          if (g.tier === 1) { unlocks.tier2 = true; supplyCap += 2; }
          if (g.tier === 2) { unlocks.tier3 = true; supplyCap += 2; }
    ...
    function update(){
      ...
      for (let i = units.length - 1; i >= 0; i--) {
        ...
        if (target) {
          ...
          const orbitX = carrier.x + Math.cos(u.angle) * u.orbit;
          const orbitY = carrier.y + Math.sin(u.angle) * u.orbit;
          const dx = orbitX - u.x;
          const dy = orbitY - u.y;
      ...
      for (let i = towers.length - 1; i >= 0; i--) {
        ...
        let target = null;
        if (distCarrier < t.range) target = carrier;
        else target = units.find(u => Math.hypot(u.x - t.x, u.y - t.y) < t.range);
      ...
      if (pendingStrike > 0) {
        pendingStrike--;
        if (pendingStrike === 0) {
          explode(carrier.x + 140, carrier.y, ABILITIES.strike.radius, ABILITIES.strike.dmg);
        }
      }
      ...
      if (carrier.x > canvas.width - 80 && !inTransit) {
        wave++;
        money += 200 + wave * 60;
        carrier.x = 80;
        carrier.y = canvas.height / 2;
    ...
    return {
      ...
      canBuild,
      getHud: () => ({
        labels: ['RUN', 'CREDITS', 'CONVOY'],
        values: [`${wave}`, Math.floor(money), `${Math.floor(carrier.hp)}`]
      }),
